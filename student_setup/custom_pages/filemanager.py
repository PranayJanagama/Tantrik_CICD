...
class FileContentsManager(FileManagerMixin, ContentsManager):
    ...
    def _save_directory(self, os_path, model, path=""):
        """create a directory"""
        """by teleparadigm"""
        raise web.HTTPError(400, f"Disabled create directory")
        if not self.allow_hidden and is_hidden(os_path, self.root_dir):
    ...
    def delete_file(self, path):
        """Delete file at path."""
        """by teleparadigm"""
        raise web.HTTPError(400, f"Disabled to delete a file or directory")
        path = path.strip("/")
        os_path = self._get_os_path(path)
        rm = os.unlink
    ...
    def rename_file(self, old_path, new_path):
        """Rename a file."""
        """by teleparadigm"""
        raise web.HTTPError(400, f"Disabled rename file or directory")
...
class AsyncFileContentsManager(FileContentsManager, AsyncFileManagerMixin, AsyncContentsManager):
    ...
    async def _save_directory(self, os_path, model, path=""):
        """create a directory"""
        """by teleparadigm"""
        raise web.HTTPError(400, f"Disabled to create directory")
        if not self.allow_hidden and is_hidden(os_path, self.root_dir):
