# if virtual environment /opt/jupyterhub/lib/python3.10/site-packages/jupyterhub/apihandlers/groups.py
# normal setup /usr/local/lib/python3.10/dist-packages/jupyterhub/apihandlers/groups.py
"""Group handlers"""
...
class GroupUsersAPIHandler(_GroupAPIHandler):
    ...
    async def delete(self, group_name):
        ...
        if len(group.users) == len(data['users']):
            self.log.info(
                    "Skipping group students deletion because of same len of requested and existing"
                )
        else:
            for user in self._usernames_to_users(data['users']):
