# /srv/nbgrader/nbgrader/nbgrader/server_extensions/assignment_list/handlers.py
...
class AssignmentList(LoggingConfigurable):
    ...
    def list_assignments(self, course_id=None):
        ...
        fetched_assignment_ids = {assignment["assignment_id"] for assignment in released["value"] if assignment["status"] == "fetched"}
        filtered_submitted = [
            group for group in submitted["value"]
            if group.get("status") == "submitted" and group["assignment_id"] in fetched_assignment_ids
        ]
    
        retvalue = {
            "success": True,
            "value": filtered_submitted + released["value"]
        }
    
        # Process each assignment to keep the latest 5 submissions
        for assignment in retvalue['value']:
            if 'submissions' in assignment:
                # Sort submissions by timestamp descending, then take first 5
                submissions = assignment['submissions']
                sorted_submissions = sorted(submissions, key=lambda s: s['timestamp'], reverse=True)
                assignment['submissions'] = sorted_submissions[:5]
